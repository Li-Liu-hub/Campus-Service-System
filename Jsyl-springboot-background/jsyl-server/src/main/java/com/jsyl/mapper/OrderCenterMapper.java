...
public interface OrderCenterMapper {
    ...
    /*查询所有活跃订单（用于推荐）*/
    @Select("SELECT o.*, u.nickname as publisherNickname FROM order_info o " +
            "LEFT JOIN user_info u ON o.user_id = u.id " +
            "WHERE o.order_status = 0 ORDER BY o.create_time DESC")
    ...
    /*统计已完成订单总金额*/
    @Select("SELECT COALESCE(SUM(order_amount), 0) FROM order_info WHERE order_status = 2")
