package com.jsyl.module.trade.service.impl;
    ...
    public void completeByUser(Long orderId, Integer userId) {
        ...
        if (order.getUserId() != null && !order.getUserId().equals(userId)) {
            Notification notification = Notification.builder()
                    ...
                    .type(3) // 订单通知
                    .title("订单已完成")
                    .content("您的订单 " + order.getOrderNo() + " 已完成，请及时评价")
                    .relatedId(orderId)
                    .isRead(0)
                    .createTime(LocalDateTime.now())
                    .build();
        ...
        if (order.getAcceptorId() != null && !order.getAcceptorId().equals(userId)) {
            Notification notification = Notification.builder()
                    .userId(order.getAcceptorId())
                    .type(3) // 订单通知
                    .title("订单已完成")
                    .content("订单 " + order.getOrderNo() + " 已完成")
                    .relatedId(orderId)
                    .isRead(0)
                    .createTime(LocalDateTime.now())
                    .build();
