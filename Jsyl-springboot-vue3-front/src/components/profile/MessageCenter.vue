...
const loadMessages = async () => {
  ...
  try {
    ...
    const type = activeTab.value === 'all' ? undefined : typeMap[activeTab.value];
    const res = await getNotificationList({ page: 1, pageSize: 100, type });
    if (res?.code === 200 && res.data) {
      messages.value = res.data.records.map((item: any) => ({
        id: item.id,
        title: item.title,
        content: item.content,
        type: item.type === 1 ? 'system' : item.type === 2 ? '互动' : 'order',
