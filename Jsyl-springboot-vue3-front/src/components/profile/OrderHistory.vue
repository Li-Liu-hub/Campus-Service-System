        ...
        >
          ...
          <div class="order-body">
            <div class="order-info">
              ...
              <!-- 显示取消申请状态 -->
              <div v-if="showCancelStatus(order)" class="cancel-status">
                <el-alert
                  :title="getCancelStatusText(order)"
                  type="warning"
                  :closable="false"
                  show-icon
                />
            ...
            <div class="order-actions" @click.stop>
              ...
              <el-button
                v-if="order.orderStatus === 1 && !isAccepted && !hasCancelRequest(order)"
                type="warning"
                size="small"
                @click="handleCancel(order)"
              >
              ...
              <el-button
                v-if="order.orderStatus === 1 && isAccepted && order.publisherCancel === 1"
                type="primary"
                size="small"
                @click="handleConfirmCancel(order)"
              >
                确认取消
              </el-button>
              <el-button
...
// 判断是否显示取消状态提示
const showCancelStatus = (order: Order): boolean => {
  // 只有已接单状态且有取消申请时才显示
  return order.orderStatus === 1 && (order.publisherCancel === 1 || order.acceptorCancel === 1);
};

// 获取取消状态文本
const getCancelStatusText = (order: Order): string => {
  if (props.isAccepted) {
    // 接单者视角
    if (order.publisherCancel === 1 && order.acceptorCancel === 1) {
      return '双方已申请取消，订单即将取消';
    } else if (order.publisherCancel === 1) {
      return '发布者申请取消订单，请确认';
    } else if (order.acceptorCancel === 1) {
      return '您已申请取消，等待发布者确认';
    }
  } else {
    // 发布者视角
    if (order.publisherCancel === 1 && order.acceptorCancel === 1) {
      ...
      return '您已申请取消，等待接单者确认';
    } else if (order.acceptorCancel === 1) {
      return '接单者申请取消订单，请确认';
    }
  }
  return '';
};
...
// 判断当前用户是否已申请取消
const hasCancelRequest = (order: Order): boolean => {
  if (props.isAccepted) {
    return order.acceptorCancel === 1;
  } else {
    return order.publisherCancel === 1;
  }
};
...
const handleCancel = async (order: Order) => {
  try {
    let confirmText = '确定要取消该订单吗？';

    // 如果订单已被接单，提示需要对方确认
    if (order.orderStatus === 1) {
      confirmText = '订单已被接单，申请取消需要对方确认，是否继续？';
    }
...
const handleConfirmCancel = async (order: Order) => {
  try {
    await ElMessageBox.confirm('对方已申请取消订单，确定同意取消吗？', '提示', {
